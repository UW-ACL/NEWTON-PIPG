function sol = solve_scs_noboundnary(pp,ppv)
%{
05/01/2022
Purnanand Elango
...
%}
    ...
    scs_data.A = [sparse(pp.n,pp.m*pp.N), sparse(kron(e_1,eye(pp.n)));      % Zero cone (initial condition)
                      % Zero cone (final condition)deleted
    ...
    scs_data.b = [pp.z0;
                  ...
                  full(ppv.g);
                  1;
                  zeros((pp.n+pp.m)*pp.N,1)];

    scs_cone = struct;
    % Dimension of zero cone
    scs_cone.z = pp.n*(pp.N+1); 
    % Box cone upper and lower bound; dimension of cone inferred from length
    scs_cone.bu = [kron(ones(pp.m*pp.N,1),pp.u_max);kron(ones(pp.n*pp.N,1),pp.x_max)];  
    scs_cone.bl = [kron(ones(pp.m*pp.N,1),-pp.u_max);kron(ones(pp.n*pp.N,1),-pp.x_max)];
