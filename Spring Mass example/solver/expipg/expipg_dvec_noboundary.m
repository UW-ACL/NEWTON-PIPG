function [x1,u1,w1,exit_flag] = expipg_dvec_noboundary(x1,u1,w1,...                                          % Main solution variables
...
%}
    ...
    for k = 1:k_max % Run algorithm for at most k_max iterations 
        ...
        t = N+1;
        x1(:,N+1) = max(x_min, min(x_max, x3(:,t) - alf*(Q*x3(:,t) + w3(:,t-1) - AT*w3(:,t))));  % Projection on singleton set
        ...
        if rem(k,k_test) == 0 % Test stopping criteria only every k_test iterations
            ...
            u2(:,1) = - R*u1(:,1) + BT*w1(:,1);            
            for t = 2:N
                x2(:,t) = - Q*x1(:,t) - w1(:,t-1) + AT*w1(:,t);
                u2(:,t) = - R*u1(:,t) + BT*w1(:,t); 
            end
            x2(:,N+1) = - Q*x1(:,N+1) - w1(:,N);
            err_p1 = 0;
            err_p1 = max(err_p1, max( (x_0 - x1(:,1)) .* max(x2(:,1),0) ));
            err_p1 = max(err_p1, max( (x_0 - x1(:,1)) .* min(x2(:,1),0) ));
            err_p1 = max(err_p1, max( (u_max - u1(:,1)) .* max(u2(:,1),0) ));
            err_p1 = max(err_p1, max( (u_min - u1(:,1)) .* min(u2(:,1),0) ));
            err_p1 = max(err_p1, norm(x1(:,2) - A*x1(:,1) - B*u1(:,1),"inf"));
            for t = 2:N+1
                err_p1 = max(err_p1, max((x_max - x1(:,t)) .* max(x2(:,t),0)));
                err_p1 = max(err_p1, max((x_min - x1(:,t)) .* min(x2(:,t),0)));
                err_p1 = max(err_p1, max((u_max - u1(:,t)) .* max(u2(:,t),0)));
                err_p1 = max(err_p1, max((u_min - u1(:,t)) .* min(u2(:,t),0)));
                err_p1 = max(err_p1, norm(x1(:,t+1) - A*x1(:,t) - B*u1(:,t),"inf"));                
            end
            err_p1 = max(err_p1);
            err_p1 = max(err_p1);            
            ...
            t = N+1;
            x2(:,N+1) = (w2(:,t-1)-AT*w2(:,t) >= 0) .* x_min + (w2(:,t-1)-AT*w2(:,t) < 0) .* x_max;
            err_d2 = 0;
            for t = 1:N
