            ...
            const array<real_T, 2U> &B, array<real_T, 2U> &C)
{
  ...
  if ((A.size(1) == 0) || (B.size(0) == 0) || (B.size(1) == 0)) {
    int32_T loop_ub;
    C.set_size(&re_emlrtRTEI, &sp, 10, B.size(1));
    loop_ub = 10 * B.size(1);
    ...
    ldc_t = (ptrdiff_t)10;
    C.set_size(&qe_emlrtRTEI, &b_st, 10, B.size(1));
            ...
            const array<real_T, 2U> &B, array<real_T, 1U> &C)
{
  ...
  if ((A.size(0) == 0) || (A.size(1) == 0) || (B.size(1) == 0)) {
    int32_T loop_ub;
    loop_ub = A.size(0);
    C.set_size(&re_emlrtRTEI, &sp, A.size(0));
    for (int32_T i{0}; i < loop_ub; i++) {
      C[i] = 0.0;
    }
  } else {
    ...
    C.set_size(&qe_emlrtRTEI, &b_st, A.size(0));
    dgemm(&TRANSA1, &TRANSB1, &m_t, &n_t, &k_t, &alpha1,
