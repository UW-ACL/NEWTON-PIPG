 ...
 *
 * dev_newtonpipg_pcg_g_emxutil.c
 *
                               ...
                               const emlrtRTEInfo *srcLocation)
{
  ...
  if (newNumel > emxArray->allocatedSize) {
    ...
    newData = emlrtMallocMex((uint32_T)i * sizeof(int16_T));
    if (newData == NULL) {
      emlrtHeapAllocationErrorR2012b(srcLocation, (emlrtCTX)sp);
    }
    if (emxArray->data != NULL) {
...
{
  emxFree_real_T(sp, &pStruct->xi);
  emxFree_real_T(sp, &pStruct->eta);
}
                               ...
                               const emlrtRTEInfo *srcLocation)
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    emxInitStruct_cell_wrap_6(sp, &pMatrix[i], srcLocation);
  }
}
...
void emxInitStruct_struct1_T(const emlrtStack *sp, struct1_T *pStruct,
                             const emlrtRTEInfo *srcLocation)
{
  emxInit_real_T(sp, &pStruct->xi, 1, srcLocation);
  emxInit_real_T(sp, &pStruct->eta, 1, srcLocation);
}
...
/* End of code generation (dev_newtonpipg_pcg_g_emxutil.c) */
